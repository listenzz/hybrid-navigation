...
`Navigation.setRoot(layout, sticky = false): Promise<boolean>` 设置应用的 UI 层级。
...
`Navigation.setRootLayoutUpdateListener(willSetRoot = () => {}, didSetRoot = () => {}): void` 监听 `setRoot` 的调用，在前后插入相应逻辑。
...
```js
// 两者都会触发 push 导航动作（sceneId 来自当前页面 props）。
// 区别在返回值：dispatch 返回 Promise<boolean>；navigator.push 返回 Promise<[number, T]>。
...
`Navigation.setInterceptor(interceptor): void` 拦截 `dispatch` 的操作，可插入登录校验、埋点等横切面逻辑。
...
[
  {
    layout: 'tabs',
    sceneId: 'tabs-scene-id',
    mode: 'normal',
    state: { selectedIndex: 0 },
    children: [
      {
        layout: 'stack',
        sceneId: 'stack-scene-id',
        mode: 'normal',
        children: [
          {
            layout: 'screen',
...
`Navigator.find(moduleName): Promise<Navigator | undefined>` 接受 `moduleName` 作为参数；页面存在时返回对应 `Navigator`，否则返回 `undefined`。
...
`present<T>(moduleName, props, options): Promise<[number, T]>` 是一种模态交互方式，类似于 Android 的 `startActivityForResult`，要求被 present 的页面返回结果给发起 present 的页面。在 iOS 中，present 表现为从底往上弹出界面。
...
`presentLayout<T>(layout: Layout): Promise<[number, T]>` present 的加强版，通过传递一个布局对象，用来 present UI 层级比较复杂的页面，同样使用 dismiss 来关闭。
...
`setResult(resultCode, data): void` 设置返回结果。结果会延迟到页面关闭时传递，因此在页面关闭之前，可以多次调用，以最后一次调用为准。

在调用 `dismiss`、`hideModal`、`pop` 等关闭页面的方法后，结果会被传递到目标页面。

### dismiss

`dismiss(): Promise<boolean>` 关闭 `present` 出来的页面，如果该页面是容器，可以在容器的任何子页面调用此方法。
...
`showModal<T>(moduleName, props, options): Promise<[number, T]>`
...
`hideModal(): Promise<boolean>` 隐藏作为 Modal 显示的页面，如果 Modal 是一个容器，可以在该容器的任何子页面调用此方法。
...
`push<T>(moduleName, props, options): Promise<[number, T]>` 打开一个新的页面，等同于 iOS 的 push 操作。
...
`pushLayout<T>(layout: Layout): Promise<[number, T]>` push 加强版，通过传递一个布局对象，展示 UI 层级比较复杂的页面。

### pop

`pop(): Promise<boolean>` 返回到前一个页面。比如你由 A 页面 `push` 到 B 页面，现在想返回到 A 页面。
...
`popTo(moduleName: string, inclusive: boolean = false): Promise<boolean>` 返回到之前的指定页面。比如你由 A 页面 `push` 到 B 页面，由 B 页面 `push` 到 C 页面，由 C 页面 `push` 到 D 页面，现在想返回 B 页面，只需要 `popTo('B')` 即可。
...
`popToRoot(): Promise<boolean>` 返回到 Stack 根页面。比如 A 页面是根页面，由 A 页面 `push` 到 B 页面，由 B 页面 `push` 到 C 页面，由 C 页面 `push` 到 D 页面，现在想返回到根部，也就是 A 页面：
...
`redirectTo(moduleName: string, props?: object, options?: NavigationItem): Promise<boolean>` 用指定页面取代当前页面，比如当前页面是 A，想要替换成 B
...
`setParams(params): void` 存放和该 navigator 相关的属性或状态，可通过 `navigator.state.params` 取出。

```js
navigator.setParams({ source: 'deeplink' });
const { source } = navigator.state.params;
```
...
`switchTab(index: number, popToRoot: boolean = false): Promise<boolean>` 切换到指定 Tab
...
`toggleMenu(): Promise<boolean>` 切换抽屉的开关状态
...
`closeMenu(): Promise<boolean>` 关闭抽屉
...
- 如果由于某些原因，需要**在页面之外**执行导航操作，可以使用 `router`，它会自动获取合适的 navigator 来执行操作，必要时关闭一些页面。
  ...
  `router.open(path, props = {}, options = {}): Promise<void>`
