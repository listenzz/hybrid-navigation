...
Android 需要继承 `HybridFragment`，你可以创建一个类似下面的类：
...
iOS 需要继承 `HBDViewController`，你可以创建一个类似下面的类：
...
```objc
// ios
HBDViewController *vc = [[HBDReactBridgeManager get] viewControllerWithModuleName:@"moduleName" props:nil options:nil];
```
...
HBDViewController *vc = [[HBDReactBridgeManager get] viewControllerWithModuleName:@"moduleName" props:props options:nil];
```
