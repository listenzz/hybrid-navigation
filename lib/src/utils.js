import { EventEmitter, EVENT_NAVIGATION, KEY_ACTION, KEY_SCENE_ID, KEY_ON, ON_BAR_BUTTON_ITEM_CLICK, } from './NavigationModule';
import store from './store';
let actionIdGenerator = 0;
function bindBarButtonItemClickEvent(item = {}, options = { inLayout: false }) {
    if (options.inLayout) {
    ...
    return JSON.parse(JSON.stringify(item, (key, value) => {
        if (key === 'action' && typeof value === 'function') {
            ...
            let event = EventEmitter.addListener(EVENT_NAVIGATION, data => {
