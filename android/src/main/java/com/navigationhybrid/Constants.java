package com.navigationhybrid;

/**
 * Created by Listen on 2018/1/15.
 */

public class Constants {

    public static final String TOP_BAR_STYLE_LIGHT_CONTENT = "light-content";
    public static final String TOP_BAR_STYLE_DARK_CONTENT = "dark-content";

    public static final String ARG_MODULE_NAME = "module_name";
    public static final String ARG_PROPS = "props";
    public static final String ARG_OPTIONS = "options";
    public static final String ARG_SCENE_ID = "sceneId";
    public static final String ARG_ACTION = "action";
    public static final String ARG_INDEX = "index";
    public static final String ARG_BADGE = "badge";
    public static final String ARG_ICON = "icon";
    public static final String ARG_ICON_SELECTED = "icon_selected";

    public static final String ACTION_SET_TAB_ICON = "SET_TAB_ICON";
    public static final String ACTION_SET_BADGE = "SET_BADGE_TEXT";
    public static final String ACTION_UPDATE_TAB_BAR = "UPDATE_TAB_BAR";

    public static final String INTENT_RELOAD_JS_BUNDLE = "reload-js-bundle";
}
